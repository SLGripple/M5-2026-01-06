...
def main():
    ...
    print("Welcome to the Calculator CLI! You know you can do this on you phone yeah?")
    
    args = parse_args()

    # Detect if interactive mode is possible
    interactive = sys.stdin.isatty()

    # If no args AND not interactive, warn the user
    if args.num1 is None and args.num2 is None and not interactive:
        print("You forgot your arguments! 🫣")
    ...
    num1 = args.num1 if args.num1 is not None else prompt_float("Enter first number: ")
    num2 = args.num2 if args.num2 is not None else prompt_float("Enter second number: ")
    op = args.op if args.op is not None else prompt_operation()
